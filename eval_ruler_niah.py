...
def build_mk_niah_sample(
    ...
    if prompt_style == "continuation":
        ...
        for k in keys:
            ...
            values_by_key.append(vals)
            for v in vals:
                # RULER official: needle uses singular ("The special magic number for X is: Y.")
                # All numeric value types map to "number" in singular form.
                _sg = {"numbers": "number", "short_int": "number", "digit2": "number",
                       "words": "word", "uuids": "uuid"}.get(value_type, value_type)
                needle_sentences.append(f"The special magic {_sg} for {k} is: {v}.")
    ...
    if prompt_style == "continuation":
        ...
        # OFFICIAL RULER template (NVIDIA/RULER scripts/data/synthetic/constants.py).
        # type_needle_v is the PLURAL form: numbers / words / uuids.
        # All numeric value types (digit2/short_int/numbers) map to "numbers".
        ...
        prompt_suffix = (
            f"\nWhat are all the special magic {_pl} for {query_str} mentioned in the provided text?"
            f" The special magic {_pl} for {query_str} mentioned in the provided text are"
        )
...
def generate_greedy(
    ...
    """
    # Rewritten to match training access pattern:
    # - Split input into [body (memorize)] + [query chunk (re-feed at each AR step)]
    # - Body is everything before the last full-query chunk.
    # - tail_chunk_len is respected only for the body's internal chunking.
    total_len = len(input_ids)
    if total_len <= chunk_size:
        # Single-chunk input: no separate body, just process and generate
        body_ids: List[int] = []
        query_ids: List[int] = list(input_ids)
        query_offset = 0
    else:
        # Multi-chunk: last chunk_size tokens is the query that gets re-fed
        query_ids = list(input_ids[-chunk_size:])
        body_ids = list(input_ids[:-chunk_size])
        query_offset = len(body_ids)

    def _build_and_write_body_memory():
        mbk = MemoryBank(max_entries=65536) if use_memory else None
        if mbk is not None:
            mbk.reset()
            if body_ids:
                body_t = torch.tensor([body_ids], dtype=torch.long, device=device)
                # Respect tail_chunk_len inside the body: last tail_chunk_len tokens
                # go last so their memory entries are freshest.
                tail_start = max(0, len(body_ids) - tail_chunk_len) if tail_chunk_len > 0 else len(body_ids)
                # Phase 1: body[:tail_start] in chunks
                for s in range(0, tail_start, chunk_size):
                    e = min(tail_start, s + chunk_size)
                    _forward_chunk(model, body_t[:, s:e], s, use_memory, mbk, True)
                # Phase 2: body[tail_start:] as a final chunk (if any)
                if tail_start < len(body_ids):
                    _forward_chunk(model, body_t[:, tail_start:len(body_ids)],
                                   tail_start, use_memory, mbk, True)
        return mbk

    # Initial pass: write body to memory, then forward query to get first logits
    mb = _build_and_write_body_memory()
    q_tensor = torch.tensor([query_ids], dtype=torch.long, device=device)
    out = _forward_chunk(model, q_tensor, query_offset, use_memory, mb, False)
    logits = out["logits"][:, -1, :]
    ...
    for i in range(max_new_tokens):
        ...
        if eos_token_id is not None and nxt_id == eos_token_id:
            break
        # Rebuild memory from body, then re-feed query+generated as ONE chunk.
        # This matches the diagnostic that achieved 9/10.
        mb = _build_and_write_body_memory()
        combined = query_ids + out_ids
        # Keep combined length <= chunk_size so RoPE cache and windowed attn behave.
        if len(combined) > chunk_size:
            drop = len(combined) - chunk_size
            combined = combined[drop:]
            eff_offset = query_offset + drop
        else:
            eff_offset = query_offset
        combined_t = torch.tensor([combined], dtype=torch.long, device=device)
        out = _forward_chunk(model, combined_t, eff_offset, use_memory, mb, False)
        logits = out["logits"][:, -1, :]
    return out_ids
